...
import { listMusicas, removeMusica } from "../services/musicas-publicas-service.js";
...
async function renderMusicas() {
  ...
  box.innerHTML = filtered.length ? filtered.map((item) => `
    <div class="admin-list-card">
      ...
      <div class="admin-list-actions">
        <a class="button-outline" href="./editor-musica-publica.html?id=${item.id}">Editar</a>
  ...
  box.querySelectorAll("[data-delete-id]").forEach((button) => {
    button.addEventListener("click", async () => {
      ...
      await recordAdminActivity({ action: "delete", module: "musicasPublicas", itemId: button.dataset.deleteId, itemName: item?.title || "Música" });
...
function applyMusicPermissions(admin) {
  ...
  if (createBtn) createBtn.classList.toggle('hidden', !hasPermission(admin, 'musicasPublicas', 'create'));
  document.querySelectorAll('.admin-list-actions .button-outline').forEach((el) => {
    el.classList.toggle('hidden', !hasPermission(admin, 'musicasPublicas', 'edit'));
  });
  document.querySelectorAll('[data-delete-id]').forEach((el) => {
    el.classList.toggle('hidden', !hasPermission(admin, 'musicasPublicas', 'delete'));
  });
}
