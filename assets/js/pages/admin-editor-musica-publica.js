...
import "../editor.js";
import { getMusica, saveMusica, findDuplicateMusicaTitle, removeMusica } from "../services/musicas-publicas-service.js";
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  document.getElementById("admin-editor-musica-form")?.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      if (duplicate) {
        ...
        if (editExisting) {
          window.location.href = `./editor-musica-publica.html?id=${duplicate.id}`;
        }
        return;
      }
  ...
  document.getElementById("delete-musica-button")?.addEventListener("click", async () => {
    ...
    try {
      ...
      await recordAdminActivity({ action: "delete", module: "musicasPublicas", itemId: musicaId, itemName: musicaName });
...
watchAuth(async (user) => {
  ...
  const canSave = isEdit ? hasPermission(admin, 'musicasPublicas', 'edit') : hasPermission(admin, 'musicasPublicas', 'create');
  const canDelete = isEdit && hasPermission(admin, 'musicasPublicas', 'delete');
