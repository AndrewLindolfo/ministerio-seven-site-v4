# Ministério Seven — aplicar navegação suave nas páginas públicas
...
foreach ($item in $publicFiles) {
  ...
  if ($content -notlike '*assets/css/seven-router.css*') {
    $content = $content.Replace('</head>', $cssLine + "`r`n</head>")
  }
  if ($content -notlike '*assets/js/seven-router.js*') {
...
Write-Host ""
Write-Host "Navegacao suave publica aplicada. Atualize com Ctrl + F5."
